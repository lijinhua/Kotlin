package com.example.koltin_core_progress.ch_2
...
fun main(args: Array<String>) {
    immutableRef()
    varAsProp() // 我们可以修改对象的属性
    varAsSideEffect() // 副作用，劲量使用val
}

fun immutableRef() {
    val x = intArrayOf(1, 2, 3)
    // x = intArray(4, 5, 6) 编译报错,因为引用不能变，所以x不能指向另一个数组，但我们可以修改x指向数组的值
    x[0] = 2
    println(x[0])
}
